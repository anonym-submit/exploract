...
for _ in range(5):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if test_ra3 > peak_ra3:
            pickle.dump(
                            dst_probs, 
                            open(f'{data_path}/network_data/dst_probs/gine_seq_{task}_best_ra3_{seed}_{test_id}_{main_size}.pickle', 'wb'), 
        ...
        if test_mrr > peak_mrr:
            pickle.dump(
                            dst_probs, 
                            open(f'{data_path}/network_data/dst_probs/gine_seq_{task}_best_mrr_{seed}_{test_id}_{main_size}.pickle', 'wb'), 
                            protocol=pickle.HIGHEST_PROTOCOL
                        )
            peak_mrr = test_mrr
...
pickle.dump(
    results, 
    open(f'{data_path}/network_data/model_stats/{task}_{seed}_{main_size}_{test_id}_gine_seq.pickle', 'wb'), 
    protocol=pickle.HIGHEST_PROTOCOL
)  
